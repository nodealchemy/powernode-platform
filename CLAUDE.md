...
- **Extension Isolation**: each `extensions/*` is self-contained. Extensions depend on core; **core NEVER depends on extensions**. Enforced by the blocking `core-purity-check.sh` **and** a model-agnostic `scripts/pattern-validation.sh` scan mirror (not just the Claude hook) — core source must not reference a private extension by name (its `Namespace::`, submodule path, or import alias), and since 2026-09-02 an extension's source must not reference a DIFFERENT extension either (the hook resolves the file's own slug and checks every other one; pre-existing references are grandfathered in `.claude/hooks/core-purity-baseline.txt`, private-path entries in the gitignored `.local.txt`); both derive the forbidden names dynamically from `extensions/private/*`. Route through a generic seam instead.
