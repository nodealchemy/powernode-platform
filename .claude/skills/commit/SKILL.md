...
1. **Extension file leak**: Check `git status` output for any files under `extensions/*/` staged in the parent repo. These MUST be committed inside their submodule, not the parent.
...
For each extension submodule with changes (iterate `extensions/*/`):
...
- `for d in extensions/*/; do echo "== $d =="; git -C "$d" log --oneline -5; done` — show each extension's commits (if changes)
