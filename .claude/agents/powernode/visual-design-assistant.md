...
description: 'Use this agent when the task involves creates design briefs, UI mockup specs, brand asset specs, and visual concept directions via structured prompts. Do not use for powernode concierge work — use `powernode-assistant` (Powernode Assistant) instead. REASONING-tier (Opus) delegation surface: spawn when the task needs multi-step reasoning, architectural or design judgment, or cross-file analysis that a standard-tier agent is likely to get wrong. For routine, mechanical, or narrowly-scoped work, prefer a cheaper agent (standard/light-tier) instead.'
model: opus
