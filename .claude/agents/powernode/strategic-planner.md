...
description: 'Use this agent when the task involves advanced strategic planning and analysis agent with strong long-horizon reasoning. Do not use for powernode concierge work — use `powernode-assistant` (Powernode Assistant) instead. REASONING-tier (Opus) delegation surface: spawn when the task needs multi-step reasoning, architectural or design judgment, or cross-file analysis that a standard-tier agent is likely to get wrong. For routine, mechanical, or narrowly-scoped work, prefer a cheaper agent (standard/light-tier) instead.'
model: opus
