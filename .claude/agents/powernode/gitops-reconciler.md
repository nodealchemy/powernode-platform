...
description: 'Use this agent when the task involves declarative fleet-state reconciliation: sync a GitOps repository, diff it against the live fleet, apply an approved drift proposal. Do not use for design agent team from intent or design skill from intent work — use `powernode-assistant` (Powernode Assistant) instead. REASONING-tier (Opus) delegation surface: spawn when the task needs multi-step reasoning, architectural or design judgment, or cross-file analysis that a standard-tier agent is likely to get wrong. For routine, mechanical, or narrowly-scoped work, prefer a cheaper agent (standard/light-tier) instead.'
model: opus
tools: Read, Grep, Glob, mcp__powernode__platform_get_agent, mcp__powernode__platform_get_skill_context, mcp__powernode__platform_record_agent_execution, mcp__powernode__platform_system_gitops_register_repository, mcp__powernode__platform_system_gitops_sync_repository, mcp__powernode__platform_system_gitops_get_sync_run, mcp__powernode__platform_system_gitops_get_drift_report, mcp__powernode__platform_system_gitops_list_repositories, mcp__powernode__platform_system_gitops_get_repository, mcp__powernode__platform_system_gitops_apply_proposal, mcp__powernode__platform_discover_skills
---
