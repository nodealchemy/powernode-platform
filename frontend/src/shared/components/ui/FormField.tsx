...
export const FormField = forwardRef<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement, FormFieldProps>(
  ({ 
    ...
    const errorClasses = error 
      ? 'border-theme-error-border focus:border-theme-error-border focus:shadow-[0_0_0_4px_rgba(var(--color-error-rgb),0.1)] hover:border-theme-error-hover' 
      : '';
    ...
    return (
      <div className={`${floatingLabel ? 'relative' : 'space-y-2'}`}>
        {!floatingLabel && (
          <label className={labelClasses}>
            <span className="flex items-center gap-1">
              {label}
              {required && (
                <span className="text-theme-error-fg animate-pulse">*</span>
              )}
            </span>
          </label>
        )}
        ...
        {error && (
          <p className="text-sm text-theme-error-fg flex items-center gap-1 mt-1 animate-in slide-in-from-top-1 duration-200">
