...
/**
 * useCascadingOptions - Hook for managing cascading select options with async loading
 *
 * Usage:
 * const { levels, setLevelOptions, updateLevelOptions } = useCascadingOptions([
 *   { name: 'provider', label: 'Provider', options: providers },
 *   { name: 'region', label: 'Region', options: [] },
 *   { name: 'zone', label: 'Availability Zone', options: [] }
 * ]);
 */
export function useCascadingOptions(initialLevels: CascadingLevel[]) {
  const [levels, setLevels] = useState<CascadingLevel[]>(initialLevels);

  const setLevelOptions = useCallback((levelName: string, options: CascadingOption[]) => {
    setLevels(prev => prev.map(level =>
      level.name === levelName
        ? { ...level, options, loading: false }
        : level
    ));
  }, []);

  const setLevelLoading = useCallback((levelName: string, loading: boolean) => {
    setLevels(prev => prev.map(level =>
      level.name === levelName
        ? { ...level, loading }
        : level
  ...
  const updateLevelOptions = useCallback(async (
    levelName: string,
    fetchOptions: () => Promise<CascadingOption[]>
  ) => {
    setLevelLoading(levelName, true);
    try {
      const options = await fetchOptions();
      setLevelOptions(levelName, options);
    } catch {
      setLevelOptions(levelName, []);
    }
  }, [setLevelLoading, setLevelOptions]);
