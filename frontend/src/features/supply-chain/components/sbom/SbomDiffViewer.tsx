  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-4">
          <div className="flex items-center gap-2 text-theme-success-fg mb-2">
            <Plus className="w-5 h-5" />
            <span className="font-medium">Added</span>
          </div>
          <p className="text-3xl font-bold text-theme-success-fg">{diff.added_count}</p>
          <p className="text-sm text-theme-secondary">components</p>
        </Card>

        <Card className="p-4">
          <div className="flex items-center gap-2 text-theme-error-fg mb-2">
            <Minus className="w-5 h-5" />
            <span className="font-medium">Removed</span>
          </div>
          <p className="text-3xl font-bold text-theme-error-fg">{diff.removed_count}</p>
          <p className="text-sm text-theme-secondary">components</p>
        </Card>

        <Card className="p-4">
          <div className="flex items-center gap-2 text-theme-warning-fg mb-2">
            <RefreshCw className="w-5 h-5" />
            <span className="font-medium">Changed</span>
          </div>
          <p className="text-3xl font-bold text-theme-warning-fg">{diff.changed_count}</p>
          <p className="text-sm text-theme-secondary">components</p>
        </Card>
      </div>
      ...
      {diff.added_components.length > 0 && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-theme-primary mb-4 flex items-center gap-2">
            <Plus className="w-5 h-5 text-theme-success-fg" />
          ...
          <div className="space-y-2">
            {diff.added_components.map((component, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 bg-theme-success-fg/10 rounded-lg"
              >
                <div className="flex items-center gap-3">
                  <Package className="w-4 h-4 text-theme-success-fg" />
      ...
      {diff.removed_components.length > 0 && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-theme-primary mb-4 flex items-center gap-2">
            <Minus className="w-5 h-5 text-theme-error-fg" />
          ...
          <div className="space-y-2">
            {diff.removed_components.map((component, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 bg-theme-error-fg/10 rounded-lg"
              >
                <div className="flex items-center gap-3">
                  <Package className="w-4 h-4 text-theme-error-fg" />
      ...
      {diff.changed_components.length > 0 && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold text-theme-primary mb-4 flex items-center gap-2">
            <RefreshCw className="w-5 h-5 text-theme-warning-fg" />
          ...
          <div className="space-y-2">
            {diff.changed_components.map((component, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 bg-theme-warning-fg/10 rounded-lg"
              >
                <div className="flex items-center gap-3">
                  <Package className="w-4 h-4 text-theme-warning-fg" />
                  <span className="font-medium text-theme-primary">{component.name}</span>
                  <span className="text-theme-error-fg line-through">@{component.old_version}</span>
                  <span className="text-theme-secondary">→</span>
                  <span className="text-theme-success-fg">@{component.new_version}</span>
                </div>
                <Badge variant="outline" size="sm">{component.ecosystem}</Badge>
              </div>
            ))}
      ...
      {(diff.added_vulnerabilities.length > 0 || diff.removed_vulnerabilities.length > 0) && (
        <Card className="p-6">
          ...
          {diff.added_vulnerabilities.length > 0 && (
            <div className="mb-4">
              <h4 className="text-sm font-medium text-theme-error-fg mb-2">
              ...
              <div className="flex flex-wrap gap-2">
                {diff.added_vulnerabilities.map((vuln, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 p-2 bg-theme-error-fg/10 rounded"
                  >
          ...
          {diff.removed_vulnerabilities.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-theme-success-fg mb-2">
              ...
              <div className="flex flex-wrap gap-2">
                {diff.removed_vulnerabilities.map((vuln, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 p-2 bg-theme-success-fg/10 rounded"
                  >
