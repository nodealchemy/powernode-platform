...
export const BudgetTransactionHistory: React.FC<BudgetTransactionHistoryProps> = ({ budgetId, currency }) => {
  ...
  if (isLoading) {
    return <p className="text-sm text-theme-tertiary py-2">Loading transactions...</p>;
  }
  ...
  if (transactions.length === 0) {
    return <p className="text-sm text-theme-tertiary py-2">No transactions recorded yet.</p>;
  }

  return (
    <div className="space-y-2">
      <h4 className="text-xs font-semibold text-theme-tertiary uppercase tracking-wide">Transaction History</h4>
        ...
        {transactions.map((txn: BudgetTransaction) => {
          ...
          return (
            <div key={txn.id} className="flex items-center gap-3 py-1.5 text-sm">
              <TxnIcon className="h-4 w-4 text-theme-tertiary shrink-0" />
              ...
              <span className="text-theme-tertiary text-xs ml-auto">
