    ...
    >
      <div className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div
              className={`flex items-center gap-3 p-4 rounded-lg ${
                result.success
                  ? 'bg-theme-success-fg/10 dark:bg-theme-success-fg/20 border border-theme-success-border/30 dark:border-theme-success-border/50'
                  : 'bg-theme-danger-fg/10 dark:bg-theme-danger-fg/20 border border-theme-danger-border/30 dark:border-theme-danger-border/50'
              }`}
            >
              {result.success ? (
                <CheckCircle className="h-8 w-8 text-theme-success-fg flex-shrink-0" />
              ) : (
                <XCircle className="h-8 w-8 text-theme-danger-fg flex-shrink-0" />
              )}
            ...
            {result.details && (
              <div className="bg-theme-surface border border-theme rounded-lg p-4 space-y-3">
                ...
                {result.details.readable !== undefined && (
                  <div className="flex items-center justify-between">
                    ...
                    {result.details.readable ? (
                      ...
                      <XCircle className="h-5 w-5 text-theme-danger-fg" />
                    )}
                  </div>
                )}
                ...
                {result.details.error && (
                  <div className="mt-3 p-3 bg-theme-danger-fg/10 dark:bg-theme-danger-fg/20 border border-theme-danger-border/30 dark:border-theme-danger-border/50 rounded-lg">
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-theme">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-theme-primary bg-theme-surface border border-theme rounded-lg hover:bg-theme-surface-hover transition-colors"
          >
            Close
          </button>
        </div>
