...
// The *_EMAIL/*_PASSWORD env vars are provisioned per run in cypress.config.ts
// (setupNodeEvents -> scripts/e2e/provision-test-logins.cjs). No credentials file.
const getCredentials = (role?: string): RoleCredentials => {
  ...
  // Fallback: should never be used in practice — cypress.config.ts provisions
  // per-run logins into Cypress.env before specs run.
  console.warn(`⚠️ Using fallback credentials for ${prefix}. Ensure the backend is up and demo users are seeded (POWERNODE_SEED_DEMO=true rails db:seed).`);
  return {
