...
agents_data.each do |ad|
  agent = Ai::Agent.find_or_create_by(account: admin_account, name: ad[:name]) do |a|
    ...
    # The per-agent provider is the team's mixed-provider showcase; the seam
    # keeps it whenever the family rule allows and refuses one that could not
    # run this definition's pin (nil rather than an invalid row).
