...
RSpec.describe 'Api::V1::Ai::AiOps', type: :request do
  ...
  # fc-42 review item 0: `permissions:` above mints an AD-HOC role holding
  # exactly those literal strings — it never proves a REAL, catalog-synced
  # role (Role.sync_from_config!, from Permissions.permissions_for_role)
  # actually grants ai.aiops.read. ai.aiops.read is generated by the
  # `resource :aiops, actions: %i[read write]` DSL call in permissions.rb
  # (namespace "ai"), not a literal hash entry — grep for the string misses it
  # (fc-40 precedent). Verified with Permissions.permission_exists?/
  # permissions_for_role first: it exists, granted to exactly
  # admin/ai_specialist/manager/owner, the same roster ai.analytics.read
  # grants to — no permissions.rb change needed. Proven here through a REAL
  # "manager" role rather than re-deriving the same conclusion from the DSL.
  describe 'a non-admin ROLE (not an ad-hoc permission list) reaching the AIOps dashboard' do
    let(:manager) { create(:user, :manager, account: account) }
    let(:manager_headers) { auth_headers_for(manager) }

    it 'returns 200 for a manager, the same roster ai.analytics.read grants to' do
      allow(service).to receive(:aiops_dashboard).and_return({})

      get '/api/v1/ai/aiops/dashboard', headers: manager_headers
