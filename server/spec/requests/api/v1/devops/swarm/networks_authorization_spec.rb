...
# Authorization gate for swarm networks: reads (index/show) ->
# devops.swarm.read; mutations (create/destroy) -> devops.swarm.manage.
RSpec.describe "Api::V1::Devops::Swarm::Networks authorization", type: :request do
  ...
  let(:network_manager_stub) { instance_double(::Devops::Docker::NetworkManager) }

  before do
    allow(::Devops::Docker::NetworkManager).to receive(:new).and_return(network_manager_stub)
    allow(network_manager_stub).to receive(:list).and_return([])
    allow(network_manager_stub).to receive(:create).and_return({ "Id" => "net" })
  end

  describe "no permission" do
    it "forbids a read (index)" do
      get "/api/v1/devops/swarm/clusters/#{cluster.id}/networks",
          headers: auth_headers_for(no_perm), as: :json
      expect(response).to have_http_status(:forbidden)
    end

    it "forbids a mutation (create)" do
      post "/api/v1/devops/swarm/clusters/#{cluster.id}/networks",
           params: { network: { name: "net", driver: "overlay" } },
           headers: auth_headers_for(no_perm), as: :json
      expect(response).to have_http_status(:forbidden)
    end
  end
