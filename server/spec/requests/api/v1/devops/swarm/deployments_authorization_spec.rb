...
# Authorization gate for swarm deployments: read-only controller
# (index/show) -> devops.swarm.read. No mutations, so no manage gate.
RSpec.describe "Api::V1::Devops::Swarm::Deployments authorization", type: :request do
  ...
  describe "no permission" do
    it "forbids a read (index)" do
      get "/api/v1/devops/swarm/clusters/#{cluster.id}/deployments",
