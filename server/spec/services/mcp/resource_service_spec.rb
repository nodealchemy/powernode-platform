...
RSpec.describe Mcp::ResourceService do
  ...
  # user-facing resources controller. Routed through
  ...
  describe '#read_resource (stdio transport)' do
    ...
    it 'executes a whitelisted stdio command via the worker and parses a successful resources/read response' do
      ...
      expect(Mcp::WorkerStdioClient).to receive(:execute) do |account_id:, server:, mcp_request:|
        ...
        expect(mcp_request[:method]).to eq('resources/read')
    ...
    # inside #read_resource, not its outer `rescue StandardError`. Same
    ...
    # propagate unrescued from #send_stdio_request, so #read_resource's
