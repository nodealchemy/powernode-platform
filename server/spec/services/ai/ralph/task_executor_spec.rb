...
RSpec.describe Ai::Ralph::TaskExecutor, type: :service do
  ...
  describe '#build_agent_options (effort mapping — LIVE, ungated)' do
    ...
    it 'emits effort regardless of the Fable framework toggle (not gated by it)' do
      allow(Ai::FableRouting).to receive(:enabled_for?).and_return(false)
      opts = executor.send(:build_agent_options, agent, provider, messages)
      expect(opts).to have_key(:effort)
    end
