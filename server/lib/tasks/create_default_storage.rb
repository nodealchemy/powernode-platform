...
# Script to create default local storage for all accounts
puts "🗄️  Creating default local storage providers..."

Account.find_each do |account|
  # Skip if account already has a default storage
  if account.file_storages.where(is_default: true).exists?
    puts "  ↪ Account '#{account.name}' already has default storage"
    next
  end
  ...
  begin
    ...
    puts "  ✅ Created default storage for '#{account.name}' (ID: #{storage.id})"
  rescue => e
    puts "  ❌ Failed to initialize storage for '#{account.name}': #{e.message}"
    storage.update(status: "failed", health_status: "failed")
  end
end

puts "✅ Default storage creation complete!"
