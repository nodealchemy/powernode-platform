  ...
  class RegistryService
    ...
    class << self
      ...
      def list_templates(filters: {}, page: 1, per_page: 20)
        scope = Devops::IntegrationTemplate.all
      ...
      def find_template(identifier)
        template = if uuid?(identifier)
          Devops::IntegrationTemplate.find_by(id: identifier)
        else
          Devops::IntegrationTemplate.find_by(slug: identifier)
        end
      ...
      def create_template(attributes)
        template = Devops::IntegrationTemplate.new(attributes)
      ...
      def list_instances(account:, filters: {}, page: 1, per_page: 20)
        scope = Devops::IntegrationInstance.where(account: account)

        scope = scope.where(status: filters[:status]) if filters[:status].present?
        scope = scope.joins(:template)
                     .where(devops_integration_templates: { integration_type: filters[:type] }) if filters[:type].present?
      ...
      def find_instance(account:, instance_id:)
        instance = Devops::IntegrationInstance
      ...
      def create_credential(account:, attributes:, created_by: nil)
        credential = Devops::IntegrationCredential.new(
          account: account,
          created_by_user: created_by,
        ...
        # The secret material is assigned BEFORE the one save: the model
        # validates encrypted_credentials / encryption_key_id presence, so
        # saving first (then assigning) failed every create (IMP-01a04d08-9288).
        # `.to_h`: the controller hands over permitted ActionController::Parameters,
        # which is not a Hash, and the model's credentials_format rejects it.
      ...
      def search_templates(query:, filters: {}, page: 1, per_page: 20)
        scope = Devops::IntegrationTemplate.where(is_public: true)
      ...
      def integration_types
        Devops::IntegrationTemplate::INTEGRATION_TYPES
      end
      ...
      def generate_slug(name, account)
        ...
        while Devops::IntegrationInstance.exists?(account: account, slug: slug)
      ...
      def build_executor(instance)
        Devops::ExecutionService.build_executor(instance)
      end
    end
  end
end
