  ...
  module Docker
    class ApiClient
      ...
      def configure_tls(conn)
        ...
        # TlsCredentialParams#build_tls_credentials packs the blob whenever
        # ANY of tls_ca/tls_cert/tls_key is present, not all three. An
        # asymmetric pair (only one of client_cert/client_key present) is
        # never legitimate, so it's rejected loudly instead of silently
        # falling back to CA-only (which would look like working TLS while
        # quietly skipping the mutual-auth the operator configured).
        client_cert = @tls_credentials[:client_cert]
        client_key = @tls_credentials[:client_key]
        if client_cert.present? && client_key.present?
          conn.ssl.client_cert = OpenSSL::X509::Certificate.new(client_cert)
          conn.ssl.client_key = OpenSSL::PKey.read(clean_pem_key(client_key))
        elsif client_cert.present? || client_key.present?
          raise ConnectionError.new("Invalid TLS credentials: client_cert and client_key must both be present, or both absent for server-verify-only TLS")
        end
