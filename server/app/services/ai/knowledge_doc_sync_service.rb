...
module Ai
  class KnowledgeDocSyncService
    # Platform-scope outputs land in docs/reference/auto/ (see docs/reference/auto/manifest.yml).
    # Extension-scope outputs continue to land in extensions/<slug>/docs/knowledge/.
    PLATFORM_OUTPUT_DIR = Rails.root.join("..", "docs", "reference", "auto")
    PLATFORM_TODO_DIR = Rails.root.join("..", "docs", "reference", "auto")
    EXTENSIONS_ROOT = Rails.root.join("..", "extensions")

    # Per-scope filename map: platform uses lowercase-hyphenated under docs/reference/auto/;
    # extensions keep the legacy UPPERCASE names inside extensions/<slug>/docs/knowledge/.
    PLATFORM_FILENAMES = {
    ...
    # Resolve per-scope filename: platform → lowercase-hyphenated under docs/reference/auto/;
    # extensions → legacy UPPERCASE under extensions/<slug>/docs/knowledge/.
    def filename_for(kind, scope)
      if scope == "platform"
        PLATFORM_FILENAMES.fetch(kind)
      else
        EXTENSION_FILENAMES.fetch(kind)
      end
    end
    ...
    def render_learnings(entries, timestamp, output_dir, scope: "platform")
      ...
      write_file(filename_for(:learnings, scope), lines, output_dir)
    ...
    def render_knowledge(entries, timestamp, output_dir, scope: "platform")
      ...
      write_file(filename_for(:knowledge, scope), lines, output_dir)
      { success: true, count: entries.size }
    rescue StandardError => e
    ...
    def render_skills(entries, timestamp, output_dir, scope: "platform")
      ...
      write_file(filename_for(:skills, scope), lines, output_dir)
      { success: true, count: entries.size }
    rescue StandardError => e
    ...
    def sync_graph(timestamp)
      ...
      write_file(PLATFORM_FILENAMES[:graph], lines, PLATFORM_OUTPUT_DIR)
    ...
    def sync_todos(timestamp)
      ...
      File.write(todo_dir.join(PLATFORM_FILENAMES[:todo]), lines.join("\n") + "\n")
      Rails.logger.info("[KnowledgeDocSync] Wrote #{todo_dir.join(PLATFORM_FILENAMES[:todo])}")
      { success: true, count: entries.size }
    rescue StandardError => e
