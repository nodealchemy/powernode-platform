      ...
      module SummaryMetrics
        ...
        def real_time_metrics(force_refresh: false)
          ...
          Rails.cache.fetch(cache_key, expires_in: REAL_TIME_CACHE_TTL, force: force_refresh) do
            {
              active_executions: agent_executions.where(status: %w[running pending]).count,
        ...
        def calculate_total_cost(since)
          agent_executions.where("ai_agent_executions.created_at >= ?", since).sum(:cost_usd).to_f.round(6)
        end

        def calculate_period_cost(start_time, end_time)
          agent_executions.where(ai_agent_executions: { created_at: start_time..end_time }).sum(:cost_usd).to_f.round(6)
        end
        ...
        def calculate_avg_response_time(since)
          avg = agent_executions.where("ai_agent_executions.created_at >= ?", since)
        ...
        def pending_jobs_count
          agent_executions.where(status: %w[pending]).count
        end
      end
    end
  end
