    ...
    class DocumentGeneratorService
        ...
        [
          {
            ...
            desc: "Powernode is a subscription lifecycle management platform built with Rails 8 API backend (server/), " \
                  ...
                  "extensions/private/); when an extension is absent the app runs in single-user self-hosted " \
                  "mode with all features unlocked.",
            model_prefixes: ["Root"],
            route_prefixes: ["root"],
      ...
      def generate_conventions_reference
        { name: "Development Conventions Reference", category: "detail", content: <<~MD }
          ...
          - Optional features via git-submodule extensions under `extensions/` (private under `extensions/private/`)
