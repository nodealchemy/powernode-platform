...
module Mcp
  ...
  # callers (#read_resource/#list_resources). A worker-side timeout now
  # arrives as `{error:{message: "...exceeded Ns..."}}` here (handled by
  # the `if response[:error]` branch in #read_resource/#list_resources)
  ...
  def send_stdio_request(request)
    ...
    begin
      ...
      @logger.error "[McpResourceService] Security violation: #{e.message}"
