...
# Ai::AgentTeamMember - Represents an agent's role and responsibilities within a team
...
module Ai
  class AgentTeamMember < ApplicationRecord
    ...
    # Delegate useful methods to agent
    delegate :name, :description, :agent_type, to: :agent, prefix: true
    delegate :account, to: :team

    # Backwards compatibility aliases (specs expect ai_agent_ prefix)
    alias_method :ai_agent_name, :agent_name
    alias_method :ai_agent_description, :agent_description
    alias_method :ai_agent_agent_type, :agent_agent_type
    ...
    def promote_to_lead!
      ...
      team.members.leads.where.not(id: id).update_all(is_lead: false)
      update!(is_lead: true)
    end
    ...
    def set_priority_order
      ...
      max_priority = Ai::AgentTeamMember
    ...
    def validate_single_team_lead
      ...
      existing_leads = team.members.leads.where.not(id: id)
    ...
    def register_with_team
      Rails.logger.info "[Ai::AgentTeamMember] Member registered: #{agent_name} as #{role} in #{team.name}"
    end

    def cleanup_team_registration
      Rails.logger.info "[Ai::AgentTeamMember] Member removed: #{agent_name} from #{team.name}"
    end
  end
end
