...
module FileManagement
  class Object < ApplicationRecord
    ...
    # Version tracking
    has_many :versions, class_name: "FileManagement::Version", foreign_key: :file_object_id, dependent: :destroy
    has_many :child_versions, class_name: "FileManagement::Object", foreign_key: "parent_file_id", dependent: :nullify

    # Sharing
    has_many :shares, class_name: "FileManagement::Share", foreign_key: :file_object_id, dependent: :destroy
    has_many :active_shares, -> { where(status: "active").where("expires_at IS NULL OR expires_at > ?", Time.current) },
             class_name: "FileManagement::Share", foreign_key: :file_object_id

    # Processing
    has_many :processing_jobs, class_name: "FileManagement::ProcessingJob", foreign_key: :file_object_id, dependent: :destroy
    has_many :pending_jobs, -> { where(status: "pending") },
             class_name: "FileManagement::ProcessingJob", foreign_key: :file_object_id
    ...
    scope :tagged_with, ->(tag_names) { joins(:tags).where(file_tags: { name: tag_names }).distinct }
    ...
    # File operations
    def url
      storage.storage_provider.file_url(self)
    end

    def download_url(expires_in: 1.hour)
      storage.storage_provider.download_url(self, expires_in: expires_in)
    end

    def signed_url(expires_in: 1.hour, disposition: "inline")
      storage.storage_provider.signed_url(self, expires_in: expires_in, disposition: disposition)
    end

    def read
      storage.storage_provider.read_file(self)
    end

    def stream(&block)
      storage.storage_provider.stream_file(self, &block)
    end

    def exists?
      storage.storage_provider.file_exists?(self)
    end
    ...
    def version_history
      versions.order(version_number: :desc)
    end
    ...
    def revert_to_version!(version_number)
      ...
      storage.storage_provider.copy_file(target_version.storage_key, storage_key)
    ...
    def validate_storage_has_space
      return unless storage && file_size

      unless storage.has_space_for?(file_size)
    ...
    def update_storage_usage
      storage.add_file_size(file_size)
    end

    def update_storage_usage_on_delete
      storage.remove_file_size(file_size) if storage
    end
