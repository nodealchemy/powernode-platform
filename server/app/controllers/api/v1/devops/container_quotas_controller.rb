...
module Api
  module V1
    module Devops
      class ContainerQuotasController < ApplicationController
        include AuditLogging

        # GET /api/v1/mcp/quotas
        def show
          quota = ::Devops::ResourceQuota.for_account(current_user.account)
          service = ::Devops::QuotaService.new(current_user.account)
        ...
        def update
          ...
          unless current_user.has_permission?("devops.container_quotas.manage")
        ...
        def usage_history
          ...
          daily_usage = account.devops_container_instances
                               .where("created_at >= ?", 30.days.ago)
                               .group("DATE(created_at)")
                               .count

          # Get hourly usage for the past 24 hours
          hourly_usage = account.devops_container_instances
