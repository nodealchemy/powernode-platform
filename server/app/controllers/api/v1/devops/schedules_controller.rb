  ...
  module V1
    module Devops
      class SchedulesController < ApplicationController
        ...
        # GET /api/v1/devops/schedules/:id
        def show
          render_success({
        ...
        def create
          ...
          pipeline = current_user.account.devops_pipelines.find(params[:schedule][:pipeline_id])
        ...
        # PATCH/PUT /api/v1/devops/schedules/:id
        def update
          # If changing pipeline, verify it belongs to the account
          if params[:schedule][:pipeline_id].present?
            current_user.account.devops_pipelines.find(params[:schedule][:pipeline_id])
          end
        ...
        # DELETE /api/v1/devops/schedules/:id
        def destroy
          @schedule.destroy!
        ...
        # POST /api/v1/devops/schedules/:id/toggle
        def toggle
        ...
        def require_read_permission
          return if current_user.has_permission?("devops.schedules.read")

          render_error("Insufficient permissions to view schedules", status: :forbidden)
        end

        def require_write_permission
          return if current_user.has_permission?("devops.schedules.write")
