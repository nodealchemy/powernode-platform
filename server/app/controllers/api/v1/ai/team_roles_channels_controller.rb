  ...
  module V1
    module Ai
      class TeamRolesChannelsController < ApplicationController
        ...
        # cleanup_messages is worker-internal — called by AiTeamMessageCleanupJob
        # (@current_user is nil for worker tokens; it operates on current_account).
        # It is NOT gated by the ai.teams.execute mutation guard (that would 403 the
        # scheduled worker, whose roles lack the perm); instead require WORKER auth
        # so a regular user cannot POST it to purge their account's messages.
        before_action -> { render_forbidden("Worker authentication required") unless worker_authenticated? },
                      only: %i[cleanup_messages]
