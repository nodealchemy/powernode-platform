    ...
    module Ai
      class PromptTemplatesController < ApplicationController
        ...
        def index
          ...
          templates = templates.where(domain: params[:domain]) if params[:domain].present?
          ...
          if params[:search].present?
            ...
            templates = templates.where("LOWER(name) LIKE ? OR LOWER(description) LIKE ?", search_term, search_term)
          end
          ...
          render_success({
            ...
            meta: {
              ...
              by_domain: prompt_templates_scope.group(:domain).count
            }
          })

          log_audit_event("ai.prompt_templates.list", current_user.account)
        ...
        def create
          ...
          if template.save
            ...
            log_audit_event("ai.prompt_templates.create", template)
          else
        ...
        def update
          if @prompt_template.update(prompt_template_params)
            ...
            log_audit_event("ai.prompt_templates.update", @prompt_template)
          else
        ...
        def destroy
          ...
          log_audit_event("ai.prompt_templates.delete", @prompt_template)
        rescue StandardError => e
        ...
        # POST /api/v1/ai/prompt_templates/:id/preview
        def preview
          ...
          log_audit_event("ai.prompt_templates.preview", @prompt_template)
        ...
        def duplicate
          ...
          log_audit_event("ai.prompt_templates.duplicate", new_template)
        rescue StandardError => e
        ...
        def require_read_permission
          return if current_user.has_permission?("ai.prompt_templates.read")

          render_error("Insufficient permissions to view prompt templates", status: :forbidden)
        end

        def require_write_permission
          return if current_user.has_permission?("ai.prompt_templates.write")
