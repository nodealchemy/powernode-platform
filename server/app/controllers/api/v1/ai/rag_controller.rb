  ...
  module V1
    module Ai
      class RagController < ApplicationController
        ...
        # Authorization (ai.rag.* family — RAG vector stores, DECOUPLED from the
        # kb.* article knowledge base). Placed BEFORE set_rag_service so the guard
        ...
        before_action -> { require_permission("ai.rag.read") },   only: %i[content_model index show_knowledge_base list_documents show_document query query_history list_connectors analytics update_from_source_preview]
        before_action -> { require_permission("ai.rag.create") }, only: %i[create_knowledge_base create_document create_connector perform_clone]
        before_action -> { require_permission("ai.rag.update") }, only: %i[update_knowledge_base process_document embed_chunks sync_connector update_from_source]
        before_action -> { require_permission("ai.rag.delete") }, only: %i[delete_knowledge_base delete_document]
