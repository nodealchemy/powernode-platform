...
# Redis lane. TEST_ENV_NUMBER isolates POSTGRES by name, but redis is one daemon with a
# fixed set of numbered databases, so a lane has to be a small integer AND unique among
# concurrent worktrees. It cannot be derived from the slug above: every slug this script
# generates (ext_develop, parent_develop, pp) is digitless, so any inference rule hands
# them all the same database and the suite's :suite flush wipes the other lanes' keys.
# Lane 0 belongs to the bare checkout; worktrees take 1 upward. Powernode::Redis refuses
# to run without a declared lane rather than silently sharing one, so a worktree that
# does not get one here fails loudly at spec time instead of corrupting a neighbour.
WT_REDIS_MAX_LANE=5
wt_used_lanes=""
for wt_path in $(git -C "$MAIN" worktree list --porcelain | sed -n 's/^worktree //p'); do
  [ -f "$wt_path/server/.env.test.local" ] || continue
  wt_l=$(sed -n 's/^TEST_REDIS_LANE=\([0-9][0-9]*\).*/\1/p' "$wt_path/server/.env.test.local" | head -1)
  [ -n "$wt_l" ] && wt_used_lanes="$wt_used_lanes $wt_l"
done
WT_LANE=""
wt_cand=1
while [ "$wt_cand" -le "$WT_REDIS_MAX_LANE" ]; do
  case " $wt_used_lanes " in
    *" $wt_cand "*) : ;;
    *) WT_LANE="$wt_cand"; break ;;
  esac
  wt_cand=$((wt_cand + 1))
done
...
if [ -e "$ENV_TEST_LOCAL" ]; then
  ...
  if ! grep -q '^TEST_REDIS_LANE=' "$ENV_TEST_LOCAL" 2>/dev/null; then
    warn "server/.env.test.local predates redis lane isolation — its specs will refuse to run.
       Add a lane unique among your active worktrees (0 is the bare checkout):
       echo 'TEST_REDIS_LANE=${WT_LANE:-<1-$WT_REDIS_MAX_LANE>}' >> $ENV_TEST_LOCAL"
  fi
elif [ -n "$WT_LANE" ]; then
  printf '%s\n' \
    ...
    "TEST_ENV_NUMBER=${TEST_SUFFIX}" \
    "# Redis logical database for this worktree; must be unique among concurrent worktrees." \
    "TEST_REDIS_LANE=${WT_LANE}" > "$ENV_TEST_LOCAL"
  ok "server/.env.test.local (TEST_ENV_NUMBER=${TEST_SUFFIX} → powernode_test${TEST_SUFFIX}, redis lane ${WT_LANE})"
else
  printf '%s\n' \
    "# Auto-generated by prepare-worktree.sh — isolates this worktree's TEST database." \
    "# Test DB = powernode_test${TEST_SUFFIX}; dev/prod stay shared with main." \
    "TEST_ENV_NUMBER=${TEST_SUFFIX}" \
    "# NO REDIS LANE FREE: lanes 1-${WT_REDIS_MAX_LANE} are all taken by other worktrees." \
    "# Specs here will refuse to run until one is freed (--remove a finished worktree)." > "$ENV_TEST_LOCAL"
  warn "no free redis lane (1-$WT_REDIS_MAX_LANE all in use) — specs in this worktree will refuse to run.
       Collapse a finished worktree, then add its lane here as TEST_REDIS_LANE=<n>."
fi
