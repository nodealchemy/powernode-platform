...
- **MCP- and A2A-native** — A first-class MCP server exposes the platform's tool actions to any MCP client. The generated catalog, [docs/reference/auto/mcp-tools.md](docs/reference/auto/mcp-tools.md), is the only source for what is registered and how many. Agents coordinate over the A2A protocol with agent cards.
