...
| J4 | Self-management fence (INV-1) | `self_hosting_node_id` set | an actuate path handed ops-hub's instance returns `refuse_self_managed!` (unit-level, via a dry-run signal) — never a live test against ops-hub. NOTE since IMP-b8cab7f951c7 that method WRITES a `fleet.dispatch_refused` FleetEvent, so a dry-run signal aimed at ops-hub produces a real ledger row on the live account — expect it and assert on it rather than treating the lane as side-effect-free | DEV (dry-run only) |
