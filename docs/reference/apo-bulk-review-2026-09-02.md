...
- ~~Should streamable-HTTP tools/list advertise the richer outputSchema? It hard-codes {type: object} for 2025-06-18+ clients by stated rationale (then streamable_http_controller.rb:975, moved to `Mcp::ToolCatalog#decorate_tool_entry` by IMP-7e84ae0ccc91; pinned by structured_tool_output_spec.rb:94).~~ CLOSED by IMP-b92421fb7c59 (ruling D14): `decorate_tool_entry` now takes an `output_schema:` from the caller. The platform family gets `McpPlatformToolRegistrar.default_output_schema` — the same source the ActionCable manifest uses, so the two surfaces cannot drift. The introspection family keeps a generic object schema, because `Ai::Introspection::McpToolRegistrar#execute_tool` returns the service hash directly and advertising `required: ["success"]` for it would be a new falsehood. Pre-2025-06-18 revisions still receive no `outputSchema` at all. Accepted cost: the tools/list body grows 392.1 KB → 1034.0 KB (2.64x) for a user principal at 612 registry entries.
- APO-1e prerequisites: action_category nil on all 596 new declarations (resolver defaults to require_approval); ~~FederationTool overrides #execute without super (known-inert)~~ CLOSED by IMP-149b35e5f16f, see line below; mutating/read split unpinned; private-bundle extra tool map has no acknowledgement channel now the snapshot is empty.
...
- ~~APO-1e prerequisite (from 1b review): Ai::Tools::FederationTool#execute overrides BaseTool#execute without super, bypassing the declaration registry, deny overlay and validate_params!. Must be fixed before the fail-closed flip; file as its own task.~~ **CLOSED 2026-09-02 by IMP-149b35e5f16f** (ruling D21): the dispatch body moved to `#call`, so BaseTool#execute now runs the lookup, the deny overlay and `#validate_params!` for it; the known-inert pin in `action_declaration_completeness_spec.rb` is replaced by a positive assertion that NO registry-backed class owns `#execute`. **Carried into APO-1e:** the anti-relay refusal is now also hoisted into `FederationTool#authorization_error`, because a gated action returns from BaseTool#execute's gate branch without entering `#call` — wiring `federation_invoke_tool`'s category/executor/gate_context/on_proceed must keep that hoist, or the refusal is deleted by the act of gating it and forbidden calls start parking approvals.
...
- RED SPEC ON THE BRANCH (filed): the domain-coverage spec — 13 registered categories lack a domain-table prefix after the APO-1c registrations. Will be fixed first in batch 4; do not merge before.
...
- the domain-coverage spec (now spec/requests/api/v1/ai/system_policy_domains_spec.rb):84 — 13 categories without a domain-table prefix (filed IMP-4ba48fd088ce, runs first in batch 4)
