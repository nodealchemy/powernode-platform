...
| A6b | Agent-scoped capability grant: `CapabilityMatrixService#guardrail_matrix` merges an agent-scoped `GuardrailConfig.for_agent` over the account-global matrix; the investigator agent gets an operator-visible grant for `execute_tool` whose reason names `platform.investigation.daily_cap`; prerequisite: count existing per-agent rows first (each would start taking effect). Until A6b, automatic investigations never spend: the gate refuses them honestly and they conclude on core's candidates; **prerequisite:** the spend ledger row today needs a user, so agent-attributed automatic spend must first get its own ledger row (user nil, agent and grant recorded) — no grant may let an agent spend before that row exists | a real-gate example per arm: automatic without grant refused; automatic with grant proceeds; another agent at the same tier stays refused |
...
| B5b | Core remediation lane for `provider_circuit_breaker` and `ai_provider` wrapping `Ai::SelfHealing::RemediationDispatcher` (today the only actuator gated by `self_healing_remediation`, unreachable from the router because core registers no lanes); one `RemediationDispatcher.enabled?` predicate read by the dispatcher and the lane's describe; apply via core `Ai::AutonomyGate#evaluate` (proceed/pending/blocked → auto_in_progress/awaiting_operator/not_actuatable; `gate_action!` is extension-only) with an executor whose only act is the same dispatcher entry; a per-target guard in the dispatcher (same account, action_type and target within the rate limiter's hourly window; the only prior guard was per-account). The lane acts on the account-scoped `ai_provider` row only: a shared `provider_circuit_breaker` row (null account) answers `not_actuatable` with a `SharedComponent` reason naming the provider's own row, as the fleet lane does for account-nil rows; the automatic breaker-open dispatch is unchanged | flag-off reads `not_actuatable` naming the flag; a manual request on a breaker row answers `not_actuatable` with the pointer, calls no gate and spends no consent; an automatic dispatch plus a lane apply for one target in one window yields one action; another target still runs |
...
| D1b | Run discovery where the linter exists, never in the Rails process: core asks `ExtensionRegistry.provider(:lint_discovery_executor)` (none in core mode → each unit skipped as `no_discovery_executor`; local execution deleted). The system extension leases a runner from the tenant's OWN pool (no pool → skipped with a reason; one lease per account per tick) and sends a `System::Task` to that exact NodeInstance, as native module builds do; the instance pulls a read-only repository credential over mTLS from a lease-gated `ci_lint_context` endpoint, runs a platform-owned script with the repository's own bundle, and returns linter JSON over task completion; the sweep hands it to core `DiscoveryRunService#ingest!`. Gitea label dispatch is rejected: one global runner label cannot keep a tenant's code on its own runner, and workflow inputs are unmasked | a deployed node with no dev bundle files an offer for a seeded lint violation; no credential in any payload, task record or log; the Go agent change passes two independent critics |
