...
| `tools` | `Read, Grep, Glob` (+ `Edit, Write, Bash` for `code_assistant`), the bootstrap verbs (`Ai::Tools::BootstrapVerbs::ACTIONS` — the read-only set every agent needs to obey `BASE_GUARDRAILS`, shared with the runtime tool bridge and the router since HIER-P2H) and the self-report verb, and the platform actions the agent's tool access allows — read verbs only when nothing is scoped; omitted (inherit everything) for `full_registry` agents. Always exact names: Claude Code does not resolve a prefix inside one MCP server | `Ai::ClaudeExport::ToolAllowlist`, mirroring `AgentToolBridgeService#scope_to_tool_families` |
| `## Tool families` | the families a family-scoped agent's `tools:` resolves from — every canonical seeds `tool_access.tool_families` derived from its duty surface, so no two export the same list | `ToolAllowlist.scoping_families` (rendered only for family-scoped access) |
