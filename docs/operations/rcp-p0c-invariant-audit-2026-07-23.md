...
> Direct, live, read-only queries against <pve-host>/<pve-b-host> (via `ssh admin@<pve-host>` + root cluster-trust to <pve-b-host>,
> granted by the operator after the initial audit was written) found: (1) `<pve-host>-data` is a PVE
> storage of **type `nfs`** (a self-hosted NFS re-export of a dataset under <pve-host>'s own `local-zfs`
> zpool), not the local `zfspool`-type storage the earlier text assumed — a genuinely-local
> alternative (`zfspool: local-data`) exists on <pve-host> and was NOT what ops-hub's VM was built on; (2)
> ops-hub's actual running VM 104 config (`qm config 104`) confirms its root disk, EFI disk, AND
> cloud-init drive are all on `<pve-host>-data` — i.e. **INV-6 is not just a theoretical risk, it is a live,
> confirmed violation on the currently-running control-plane VM**; (3) <pve-b-host>'s storage independence is
...
> verified-at-every-step online migration of ops-hub's three disks off `<pve-host>-data` onto `local-data`**
...
against <pve-host>/<pve-b-host> themselves** (`zpool list`, `/etc/pve/storage.cfg`, `qm config 104`,
...
- The live Proxmox provider (`<pve-provider>`, id `019e446f-916c-75d2-8f4d-b44bf7cb8664`, endpoint
...
- `ProxmoxProvider#stage_cicustom`'s own doc comment states the default assumption explicitly:
  *"Defaults assume the Powernode-platform-on-ops shape: <nas-host>-data NFS at
  /mnt/pve-data/snippets"*.
- **Live-confirmed via `qm config 104` on <pve-host>:** ops-hub's actual VM config carries
  `cicustom: user=<nas-host>-data:snippets/104-user.yml,meta=<nas-host>-data:snippets/104-meta.yml` — the
  cicustom snippets specifically ride **`<nas-host>-data`**, an NFS export from the Synology
...
- Net: **ops-hub's boot-time cloud-init / federation-payload identity delivery depends on the
  NFS-backed cicustom snippets channel (`<nas-host>-data`) being reachable at boot** — confirmed live, not
  just inferred.
...
wrong** — it inferred "<pve-host>-data is local ZFS" from the campaign design doc's own paraphrase +
provisioning metadata, without checking the actual PVE storage *type*. Live access corrected this:

**`/etc/pve/storage.cfg` on <pve-host> (full listing, live) shows `<pve-host>-data` is `nfs`-type, NOT
`zfspool`-type:**
```
zfspool: local-data
...
nfs: <pve-host>-data
	export /local-zfs/<pve-host>-data
	path /mnt/pve/<pve-host>-data
	server <pve-host>
	content vztmpl,iso,images,snippets,rootdir,import
```
`<pve-host>-data` is a **self-hosted NFS re-export** of a dataset carved out of <pve-host>'s own `local-zfs`
zpool (`local-zfs/<pve-host>-data`) — served over NFS (even to <pve-host> itself) so every cluster node can see
...
**CONFIRMED LIVE VIOLATION, not a candidate:** `qm config 104` on <pve-host> (ops-hub's actual, currently
running VM) shows:
```
scsi0: <pve-host>-data:104/vm-104-disk-0.raw,discard=on,iothread=1,size=160G     <- ROOT DISK
efidisk0: <pve-host>-data:104/vm-104-disk-0.qcow2,efitype=4m,size=528K            <- EFI/bootloader disk
ide2: <pve-host>-data:104/vm-104-cloudinit.qcow2,media=cdrom                      <- cloud-init seed drive
```
**ops-hub's root disk, EFI disk, and cloud-init drive are all on `<pve-host>-data` — the NFS-type
...
rides a self-hosted NFS re-export of <pve-host>'s zpool, not a direct local `zfspool` PVE storage. A hang
or crash in <pve-host>'s own NFS server daemon (independent of the underlying zpool's health) could stall
...
written to forbid. The underlying *bytes* being on <pve-host>'s own disks (not truly remote) does not
...
`local-data`; no `<pve-host>-data` reference anywhere in VM 104's config; `/up` and QEMU-level health clean
throughout; the cluster-wide `<pve-host>-data` reference set (100, 107, 114, 500-503, 9000) is unchanged —
this cleanup touched nothing but VM 104's own two orphaned volumes. **INV-6 is now compliant for
ops-hub.**

**<pve-b-host>'s `local-data` zpool independence — NOW DEFINITIVELY CONFIRMED, not just inferred.**
Independently verified (not merely relayed) via live, read-only queries:
- `sudo zpool list` on <pve-host>: pool `local-zfs`, **14.5T**.
- `sudo ssh <pve-b-host> zpool list` (root cluster-trust, <pve-host> → <pve-b-host>): pool `local-zfs`, **3.62T** — same
  *name* (Proxmox's conventional default), but a **different pool** — sizes differ by 4x, and:
- `sudo ssh <pve-b-host> smartctl -i /dev/disk/by-id/ata-ST4000VN008-2DR166_ZGY9R1DG`: a physical 4TB
  Seagate IronWolf (`ST4000VN008-2DR166`, serial `ZGY9R1DG`) — confirmed distinct physical hardware
  from <pve-host>'s pool. **Closed**: <pve-b-host>'s storage is an independent failure domain from <pve-host>'s, on
  distinct physical disks. (No instance is placed on <pve-b-host> today — P1-a hasn't happened — so this is
...
fabricates a fixture entry named `"<pve-host>-data"` with `plugintype: "nfs"` purely to exercise the
method's response-parsing logic — a comment was added there. In an odd twist, that fixture's
`"nfs"` plugintype turned out to match the REAL <pve-host>-data's real type after all (the comment has
...
P1 (ops-hub-B on <pve-b-host>) lands:

1. **Shared switch.** <pve-host> and <pve-b-host> (and every other fleet host) hang off the same physical network
...
2. **Site power.** Same electrical circuit/UPS domain. A power event takes <pve-host>, <pve-b-host>, and the switch
...
3. **The PVE corosync quorum itself.** The cluster is **4 nodes** (confirmed live via
   `/etc/pve/corosync.conf`): `<pve-host>` (nodeid 1, `quorum_votes: 2`), `<pve-c-host>` (nodeid 2, 1 vote), `<pve-d-host>`
   (nodeid 3, 1 vote), `<pve-b-host>` (nodeid 4, 1 vote) — **5 total votes, with <pve-host> double-weighted**.
   **Confirmed: there is currently no QDevice configured** — no `device { }` block exists in
   ...
   arbitrate life/death, and today it has no independent witness — <pve-host>'s own double vote-weight
   means <pve-host> alone is much closer to a quorum-deciding position than a peer, the opposite of the
...
4. **The NFS/snippets server itself (`<nas-host>`, Synology) — a fourth domain surfaced by this
   ...
   `ProviderVolume` (`<nas-host>-powernode`) AND (per the INV-2 finding above) the cicustom snippets
...
(<pve-host>/<pve-b-host>), one-switch, one-circuit homelab topology. The design doc's own P7 ("shrink the anchor")
...
on the live `<pve-provider>` provider **connection** going forward, scoped narrowly to that one config
key.
...
("<pve-provider>") — merge `{"cidata_transport" => "iso"}` into its `config` — or (b) explicit
...
2. **RESOLVED, same day: ops-hub's root disk, EFI disk, and cloud-init drive were confirmed LIVE on
   `<pve-host>-data` (NFS-type storage) and have since been migrated to `local-data` (genuinely local
...
4. **No QDevice configured today** (confirmed live via corosync.conf) — P1-b's witness work is a
   real, unstarted gap; <pve-host> currently carries double quorum weight with no independent tie-breaker.
...
- No live node's behavior changed as a result of the code in this increment — every new check is
  ...
  <pve-host>/<pve-b-host> during this correction pass (zpool list, storage.cfg, qm config, corosync.conf, smartctl)
