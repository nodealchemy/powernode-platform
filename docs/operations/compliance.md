...
- **Right to erasure**: `Compliance::AccountTerminationJob` (in `worker/`) is a sequence of internal API calls per user/account, not a single atomic transaction; a failure partway through reverts the termination to `grace_period` for retry (see the job's `rescue` in `process_termination`) rather than leaving a partial deletion recorded as complete. The **per-user** step works: consents/terms-acceptances/password-history are deleted, and the user record is **anonymized** in place (email/name/credentials/2FA/preferences cleared, password history purged, status set `inactive` — see `Api::V1::Internal::UsersController#anonymize`) rather than the row being deleted. The **account-level** step is currently broken: it PATCHes two URLs the router does not expose (the bare `internal/accounts/:id` status update and `subscription/anonymize`), so a termination never reaches `completed` today — tracked as a follow-up improvement. `Compliance::DataDeletionJob`'s "anonymize" deletion type anonymizes the user record the same way. Its **"full"** deletion type is also currently broken in a different way: the per-data-type endpoints it calls (`DELETE /api/v1/internal/data_deletion/:type`) are unrouted, so every per-type step fails — but the user record is anonymized regardless (that call happens after the per-type loop, inside the same method), and the request is THEN marked `failed` because of the per-type failures. A "full" erasure therefore always ends in a `failed` request status even though the user has already been anonymized by the time it does — tracked as a follow-up improvement.
