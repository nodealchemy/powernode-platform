...
provisioned across <pve-host> AND <pve-b-host> with distinct vmids (F1 working), both surviving VMs
...
| Extraction | **Perfect first pass** (2/2 runs post-IMP-019fe47a): provider `proxmox`, template `powernode-ops-cell`, regions `[<pve-host>, <pve-b-host>]`, use_case included in objective (run-a lesson) |
| Plan | 2×<pve-host> + 1×<pve-b-host> `powernode-ops-cell [uefi_disk]` + `docker_provision` (deps 1+3) — first plan to include the handshake leg |
...
| Cleanup | **RETAINED** (operator): VMs **9002** (<pve-host>) + **9008** (<pve-b-host>), both enrolled — first mission-provisioned enrolled nodes ever; useful for handshake/F5 work |

## 2. What the fixes did, live

- **F1 (v36/v37)**: concurrent steps drew **distinct vmids** (9002/9008/9009); the <pve-b-host>
  create reached PVE and the VM runs ON <pve-b-host>; instance-keyed seeds meant every VM enrolled
...
| 09:17:25 | instances up: 9002 (<pve-host>) + 9008 (<pve-b-host>) — distinct vmids, both regions |
| 09:17:43 | step 1's second instance up: 9009 (<pve-host>) |
...
- VMs **9002** (`…-1-e970…`, <pve-host>) and **9008** (`…-1-7201…`, <pve-b-host>): running, enrolled,
