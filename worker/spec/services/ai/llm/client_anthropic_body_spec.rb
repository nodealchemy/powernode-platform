...
RSpec.describe Ai::Llm::Client, '#build_anthropic_body' do
  ...
  # cache_control breakpoints BY DEFAULT so repeat agent calls re-bill ~10%
  # instead of 100% of the large static context (IMP-f702df4b0d61: a one-word
  # reply cost $1.15 at 114k uncached prompt tokens on claude-fable-5).
  describe 'prompt caching (stable-prefix cache_control)' do
    ...
    it 'caches the system prompt by default (block form with cache_control)' do
      body = client.send(:build_anthropic_body, system_messages, 'claude-fable-5')
    ...
    it 'honors cache_system_prompt: false as an opt-out (plain string system)' do
      body = client.send(:build_anthropic_body, system_messages, 'claude-fable-5', cache_system_prompt: false)
    ...
    it 'marks the LAST tool with cache_control in complete_with_tools (caches the whole tools block)' do
      ...
      client.complete_with_tools(messages: messages, tools: tools, model: 'claude-fable-5')
