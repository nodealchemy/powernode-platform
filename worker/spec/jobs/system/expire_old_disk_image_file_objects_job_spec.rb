...
RSpec.describe System::ExpireOldDiskImageFileObjectsJob, type: :job do
  ...
  describe '#execute' do
    it 'raises when the string-keyed response body reports success => false' do
      mock_api_client_success(:post, { 'success' => false, 'error' => 'storage backend unreachable' })

      expect { job_instance.execute }
        .to raise_error(/retention sweep failed: storage backend unreachable/)
    end

    it 'does not raise when the string-keyed response body reports success => true' do
      mock_api_client_success(:post, { 'success' => true, 'data' => { 'per_platform' => {} } })

      expect { job_instance.execute }.not_to raise_error
    end

    it 'sums the string-keyed per_platform retired/purged counts into the completion log' do
      mock_api_client_success(:post, {
        'success' => true,
        'data' => {
          'per_platform' => {
            'linux' => { 'retired' => 3, 'purged' => 1 },
            'windows' => { 'retired' => 2, 'purged' => 1 }
          }
        }
      })

      allow(job_instance.send(:logger)).to receive(:info)
      expect(job_instance.send(:logger)).to receive(:info).with(/swept retired=5 purged=2/)
