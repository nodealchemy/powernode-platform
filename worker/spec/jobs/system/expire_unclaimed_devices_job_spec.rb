...
RSpec.describe System::ExpireUnclaimedDevicesJob, type: :job do
  ...
  describe '#execute' do
    it 'raises when the string-keyed response body reports success => false' do
      mock_api_client_success(:post, { 'success' => false, 'error' => 'db unavailable' })

      expect { job_instance.execute }
        .to raise_error(BackendApiClient::ApiError, /db unavailable/)
    end

    it 'does not raise when the string-keyed response body reports success => true' do
      mock_api_client_success(:post, { 'success' => true, 'data' => { 'reaped_count' => 5 } })
