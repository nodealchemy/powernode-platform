...
RSpec.describe System::IdentityReaperJob, type: :job do
  ...
  describe '#execute' do
    it 'raises when the string-keyed response body reports success => false' do
      mock_api_client_success(:post, { 'success' => false, 'error' => 'grace window service down' })

      expect { job_instance.execute }
        .to raise_error(BackendApiClient::ApiError, /grace window service down/)
    end

    it 'does not raise when the string-keyed response body reports success => true' do
      mock_api_client_success(:post, { 'success' => true, 'data' => { 'reaped_users' => 2, 'reaped_groups' => 1 } })
