...
RSpec.describe AiProvisioningHandoffJob, type: :job do
  ...
  describe "#execute" do
    context "with valid params" do
      before do
        allow(api_client_double).to receive(:post).and_return(
          "success" => true,
          "data" => { "ralph_loop_id" => "loop-uuid-1", "phase" => "adapting" }
        )
      end

      it "POSTs to the internal handoff endpoint" do
        job_instance.execute(job_args)

        expect(api_client_double).to have_received(:post).with(
          "/api/v1/internal/ai/provisioning/missions/mission-uuid-1/handoff",
          {}
        )
      end

      it "returns the handoff payload including the ralph_loop_id" do
        result = job_instance.execute(job_args)
        expect(result.dig("data", "ralph_loop_id")).to eq("loop-uuid-1")
        expect(result.dig("data", "phase")).to eq("adapting")
      end
    end
    ...
    context "when the API returns success: false" do
      before do
        allow(api_client_double).to receive(:post).and_return(
          "success" => false, "error" => "ralph loop create failed"
        )
        allow(api_client_double).to receive(:patch).and_return("success" => true)
      end
      ...
      it "returns the error payload without re-raising" do
        ...
        expect(result["error"]).to eq("ralph loop create failed")
      end
    end
