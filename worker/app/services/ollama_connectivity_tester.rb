...
# Runs the Ollama connectivity diagnostic suite and computes the overall status.
#
# Owns the per-test diagnostic logic (basic connection, authentication, model
# listing/availability, inference, optional streaming, performance metrics) plus
# the tokens/sec and overall-status calculations that were previously embedded in
# OllamaConnectivityTestJob. The job now orchestrates (resolve config, report /
# persist results, telemetry) and delegates the diagnostic work here.
#
# HTTP is delegated to an injected `http_requester` that responds to
...
# circuit-breaker wrapper and structured external-API logging — so the diagnostic
# behaviour (including circuit-open handling) is preserved unchanged.
#
# Each per-test method records its outcome in @results[:tests][<name>] and #run
# returns:
#   { tests: { <name> => { status:, ... }, ... }, overall_status: <status> }
class OllamaConnectivityTester
  def initialize(test_config:, http_requester:)
    @test_config = (test_config || {}).with_indifferent_access
    @http = http_requester
    @results = { tests: {} }
  end

  # Run the full diagnostic suite and return the per-test results plus the
  # computed overall status. Streaming is opt-in, matching the original job.
  def run
    ...
    { tests: @results[:tests], overall_status: determine_overall_status }
  end
