...
module Mcp
  ...
  #
  # Owns the per-transport connection handling (stdio / websocket / http) and the
  # JSON-RPC 2.0 framing (build_mcp_request / parse_mcp_response) that was
  # previously embedded in McpToolExecutionJob. The job now orchestrates
  # (fetch tool/server, update status, report telemetry) and delegates the
  # actual transport work here.
  #
  ...
  class McpTransportClient
    ...
    # Mirrors BaseWorkerService so transport logging is consistent worker-wide.
    def logger
