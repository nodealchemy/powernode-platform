...
# Phase-0 worker job for the system_provisioning mission template.
# Wraps the server-side Ai::Provisioning::IntentCaptureService through the
# internal API. Per worker convention, the worker is API-only with the server.
class AiProvisioningCaptureIntentJob < BaseJob
  ...
  def execute(params)
    ...
    log_info("Provisioning capture_intent starting", mission_id: mission_id)
    ...
    result = backend_api_post(
      "/api/v1/internal/ai/provisioning/missions/#{mission_id}/capture_intent",
      payload
    )

    if result.is_a?(Hash) && result["success"] == false
      report_failure(mission_id, result["error"] || "capture_intent failed")
      return result
    end

    log_info("Provisioning capture_intent complete", mission_id: mission_id)
    result
  rescue StandardError => e
    log_error("AiProvisioningCaptureIntentJob failed", e, mission_id: params && params["mission_id"])
