...
module Devops
  ...
  class StepExecutionJob < BaseJob
    ...
    def fetch_step_execution(step_execution_id)
      response = api_client.get("/api/v1/internal/devops/step_executions/#{step_execution_id}")
      response.dig("data", "step_execution")
    end

    def update_execution(step_execution_id, **attributes)
      api_client.patch("/api/v1/internal/devops/step_executions/#{step_execution_id}", {
    ...
    def fetch_previous_outputs(pipeline_run_id)
      response = api_client.get("/api/v1/internal/devops/pipeline_runs/#{pipeline_run_id}/step_executions")
