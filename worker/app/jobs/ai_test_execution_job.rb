...
# executor: it clones the branch, runs the given command, and reports exit
# code + output. It does NOT decide pass/fail — the server's TestVerificationService
# parses the framework output and applies the success rule. The server only
# dispatches this when the loop's configuration enables real_test_execution.
class AiTestExecutionJob < BaseJob
  ...
  def execute(args = {})
    ...
    return if loop_id.blank? || iteration_id.blank? || command.blank?
